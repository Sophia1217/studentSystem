...
import { YsqgwList, tjById, cxById, deleteQgzxXssq } from "@/api/thrift/apply";
...
export default {
  ...
  methods: {
    showDetail(row) {
      this.$router.push({
        path: "/qgzxDetail",
        query: { id: row.id, isEdit: 2, gwid: row.gwId, isApply: flase },
      });
    },
    ...
    del() {
      deleteQgzxXssq({ ids: this.delArr }).then((res) => {
        this.query();
        this.delModal = false;
