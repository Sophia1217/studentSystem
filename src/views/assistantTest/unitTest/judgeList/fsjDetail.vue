<template>
  <div class="basicInfo">
    ...
    <div class="headLeft">
        <button class="span1" @click="handleSubmit">提交</button>
    </div>
...
<script>
import { queryWjDetail, answerWj } from "@/api/test/unitTest";
export default {
  name: "BasicInfo",
  props: [],
  data() {
  ...
  methods: {
    ...
    handleConfirm() {
      this.showSubmit = false;
      this.$set(this.form,"xghBpcr",this.queryParams.xghBpcr);
      this.$set(this.form,"xghDtr",this.$store.getters.userId);
      this.$set(this.form,"wjId",this.form.id);
      let data = {
        fdycpMkTmResList: this.mkList,
        fdycpWjjg: this.form,
      };
      answerWj(data)
        .then((res) => {
