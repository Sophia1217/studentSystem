    ...
    <div class="topDIV">
      <div class="leftDIV">
        ...
        >
          ...
          >
            <span v-if="bjzt == '1'">{{ formAdd.jzlb }}</span>
            <el-select
              v-if="bjzt == '2'"
              v-model="formAdd.jzlbEdit"
              placeholder=" 请输入评奖奖项"
              @change="jzlbChange"
              clearable
            >
          ...
          <el-form-item
            ...
            :rules="rules.zzxmmc"
          >
            <span v-if="bjzt == '1'">{{ formAdd.zzxmmc }}</span>
            <el-input
              v-if="bjzt == '2'"
          ...
          >
            <span v-if="bjzt == '1'">{{ formAdd.zzxmlb }}</span>
            <el-select
              v-if="bjzt == '2'"
              v-model="formAdd.zzxmlbEdit"
            ...
            <el-table ref="multipleTable" :data="formAdd.jxdjList">
              ...
              <el-table-column label="金额" align="center">
                <template slot-scope="scope">
                  ...
                  >
                    <span v-if="bjzt == '1'">{{ scope.row.je }}</span>
                    <el-input-number
                      size="small"
                      v-else
      ...
      <div class="rightDIV">
        ...
        <el-row :gutter="20" style="margin-top: 25px; margin-left: -22px">
          ...
          ></el-col>
          <el-col :span="4.5">
            <div v-if="bjzt == '2'" class="btns borderGreen" @click="monfp">
        ...
        <div style="margin: 20px 50px 0 -20px">
          <el-table :data="formAdd.jxfpLIST">
            ...
            <el-table-column prop="cprs" label="分配（推荐）人数">
              <template slot-scope="scope">
                <span v-if="bjzt == '1'">{{ scope.row.fprs }}</span>
                <el-input
                  v-else
                  style="width: 80px"
                  type="number"
            ...
            <el-table-column prop="cprs" label="分配金额（元）">
              <template slot-scope="scope">
                <span v-if="bjzt == '1'">{{ scope.row.fpje }}</span>
                <el-input
                  v-else
                  style="width: 80px"
                  type="number"
...
export default {
  ...
  methods: {
    ...
    test() {
      //不可兼得列表回显数据
      var resU = this.checkedCities;
      for (var i = 0; i < this.datas.length; i++) {
        if (this.datas[i].dataCode) {
          for (var j = 0; j < this.datas[i].dataCode.length; j++) {
            for (var k = 0; k < resU.length; k++) {
              if (this.datas[i].dataCode[j].dm == resU[k].dm) {
                this.datas[i].dataCode[j].checked = true;
              }
            }
          }
        }
      ...
      for (var i = 0; i < this.datas.length; i++) {
        if (this.datas[i].dataCode) {
          if (this.datas[i].dataCode.every((item) => item.checked == false)) {
            //全部为空
            this.datas[i].checked = false;
            this.datas[i].isIndeterminate = false;
          } else if (
            this.datas[i].dataCode.every((item) => item.checked == true)
          ) {
            //全部有值
            ...
            // 部分有值
            this.datas[i].checked = false;
            this.datas[i].isIndeterminate = true;
          }
        }
      }
    },
    getDetail1() {
      getDetail({ id: this.lgnSn }).then((res) => {
        ...
        this.formAdd.jxfpLIST = res.data.rcswJzsqJxfpszReqList;
        this.formAdd.sfyxcftj = Number(this.formAdd.sfyxcftj);
        this.formAdd.zbxhfs = Number(this.formAdd.zbxhfs);
        this.jzlbChange(this.formAdd.jzlbm);
        //不可兼得列表回显数据
        this.checkedCities = res.data.rcswJzsqJxbkjdszList;
      });
    },
    monfp() {
        ...
        if (this.formInner.pyccDefList[0].tjmzList.length > 0) {
          ...
          var data = {
            ...
            rcswJzsqJxjbszReq: {
              ...
              zzxmlbm: this.formAdd.zzxmlbEdit, // 奖助类别
