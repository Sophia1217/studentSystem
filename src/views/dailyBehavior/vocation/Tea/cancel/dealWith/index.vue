...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      this.queryParams.xh = this.select == "xh" ? this.searchVal : "";
      this.queryParams.qjts = this.select == "qjts" ? this.searchVal : -1;
      this.queryParams.xnList = [this.dqXnxq[0]];
      this.queryParams.xqmList = [this.dqXnxq[1]];
      getXjYclList(this.queryParams)
        .then((response) => {
          this.basicInfoList = response.data; // 根据状态码接收数据
          this.total = response.totalCount; //总条数
    ...
    handleConfirm() {
      ...
      excelLsXjYclList(this.exportParams)
        .then((res) => {
          this.downloadFn(res, "销假列表已处理列表导出.xlsx", "xlsx");
