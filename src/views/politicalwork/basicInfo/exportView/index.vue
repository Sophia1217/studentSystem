...
<script>
import { exportQuery, exportBasicInfo } from "@/api/politicalWork/basicInfo";
export default {
  name: "exportView",
    ...
    handleConfirm() {
      ...
      var arr =
        this.multipleSelection.length > 0
          ? this.multipleSelection.map((item) => item.gh)
          : [];
      this.$set(this.manageRegStuInfoParam, "ghList", arr);
      console.log(this.multipleSelection);
      var params = {
        // hxList: hxList,
        columnInfoList: columnInfoList,

        staffListParms: this.manageRegStuInfoParam,
      };
      exportBasicInfo(params).then((res) => {
        //勾选一条，导出文件名为学号姓名
        if (this.multipleSelection.length == 1) {
          let exportXh = this.multipleSelection[0].gh;
          let exportXm = this.multipleSelection[0].xm;
          this.downloadFn(res, exportXh + exportXm, "xls");
        } else {
          this.downloadFn(res, "政工干部信息表", "xlsx");
        }
      });
    },
  },
