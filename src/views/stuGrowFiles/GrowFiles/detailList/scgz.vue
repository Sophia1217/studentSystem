      ...
      <div class="mt15">
        ...
        >
          ...
          <el-table-column
            ...
            width="50"
          ></el-table-column>
          <el-table-column prop="hdssmc" label="活动/赛事名称" min-width="140px"
      ...
      >
        <el-form ref="formAdd" :model="formAdd" :rules="rules">
          <el-table :data="formAdd.addData">
            ...
            <el-table-column label="参赛凭证" align="center" :render-header="addRedStar">
              <template slot-scope="scope">
                <el-upload
                  action="#"
                  multiple
    ...
    <div class="tableWrap mt15">
      <div class="headerTop">
        ...
        <div class="headerRight">
          ...
          <div class="btns borderGreen" @click="submitCy">
            <i class="icon tjIcon"></i><span class="title1">提交</span>
          </div>
        </div>
      </div>
      ...
      <el-dialog title="提交" :visible.sync="submitModalCy" width="30%">
        <template>
          <div>
            <span>创业经历确认提交？</span>
          </div>
        ...
        <span slot="footer" class="dialog-footer">
          <el-button @click="subCancelCy">取 消</el-button>
          <el-button type="primary" class="confirm" @click="submitConfirmCy"
            >确 定</el-button
          >
        </span>
      </el-dialog>
...
<script>
import { editHd, delHd, queryHd, tjHd, backHd,
  editCy, delCy, queryCy, tjCy, backCy,
...
/*----------------分界线-----后面是创业经历------------------------------*/
    ...
    submitCy() {
      var falg = 1;
      for (var i = 0; i < this.valCy.length; i++) {
        if (this.valCy[i].status !== "01") falg = 2;
      }
      if (falg == 1) {
        if (this.subArrCy && this.subArrCy.length > 0) {
          this.submitModalCy = true;
        } else {
          this.$message.error("请先勾选数据");
        }
      } else {
    ...
    // 判断 起始时间 结束时间
    changeDate(flag) {
      let addParams = this.formAddCy.addDataCy[0];
      let editParams = this.formEditCy.editDataCy[0];
      if (flag==1) {//新增起始时间
        if (addParams.jssj) {
