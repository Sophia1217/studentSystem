    ...
    <div class="tableWrap mt15">
      <div class="headerTop">
        <div class="headerLeft">
          <span class="title">等级证书</span> <i class="Updataicon"></i>
        </div>
      ...
      <div class="mt15">
        ...
        >
          ...
          <el-table-column prop="zf" label="总分（成绩）" sortable="custom">
          </el-table-column>
          <el-table-column
            prop="zsbh"
            label="证书编号"
          ...
          <el-table-column prop="fzrq" label="发证日期" sortable="custom">
          </el-table-column>
          <el-table-column
            prop="fileList"
            label="支撑材料"
        ...
        <el-form ref="formAdd" :model="formAdd" :rules="rules">
          <el-table :data="formAdd.addData">
            ...
            >
              <template slot-scope="scope">
                <el-form-item
                  :prop="'addData.' + scope.$index + '.zslxm'"
                  :rules="rules.zslxm"
                >
                  <el-select v-model="scope.row.zslxm" placeholder="请选择">
                    <el-option
                      v-for="(item, index) in dmxszslxm"
                      :key="index"
                      :label="item.mc"
                      :value="item.dm"
                    ></el-option>
            ...
            >
              <template slot-scope="scope">
                <el-form-item
                  :prop="'addData.' + scope.$index + '.fzrq'"
                  :rules="rules.fzrq"
                >
                  <el-date-picker
                    v-model="scope.row.fzrq"
                    type="date"
        ...
        <el-form ref="formEdit" :model="formEdit" :rules="rules">
          <el-table :data="formEdit.editData">
            ...
            >
              <template slot-scope="scope">
                <el-form-item
                  :prop="'editData.' + scope.$index + '.zslxm'"
                  :rules="rules.zslxm"
                >
                  <el-select v-model="scope.row.zslxm" placeholder="请选择">
                    <el-option
            ...
            >
              <template slot-scope="scope">
                <el-form-item
                  :prop="'editData.' + scope.$index + '.fzrq'"
                  :rules="rules.fzrq"
                >
                  <el-date-picker
                    v-model="scope.row.fzrq"
...
import {
  ...
  del,
  query,
} from "@/api/stuDangan/detailList/djzs";
  ...
  data() {
    return {
      ...
      rules: {
        ...
        // kmm: [{ required: true, message: "科目不能为空", trigger: "blur" }],
        dj: [{ required: true, message: "等级不能为空", trigger: "blur" }],
        zf: [{ required: true, message: "总分（成绩）不能为空", trigger: "blur" }],
        fzdw: [
          {
            required: true,
            message: "发证单位不能为空",
        ...
        zsbh: [{ required: true, message: "证书编号不能为空", trigger: "blur" }],
      },
    };
  },
  mounted() {
  ...
  methods: {
      ...
      getCodeInfoByEnglish(data).then((res) => {
        switch (val) {
          case "dmsplcm": //审批结果
            ...
            break;
          case "dmxszslxm":
            this.dmxszslxm = res.data; //培训单位类别码_公用
            break;
        }
      });
    },
    ...
    editClick() {
      if (!this.checkFormEdit()) {
        ...
        formData.append("zslxm", data.zslxm);
        formData.append("kmm", data.kmm ||"");
        formData.append("dj", data.dj);
        formData.append("pxjssj", data.pxjssj);
        formData.append("zf", data.zf);
