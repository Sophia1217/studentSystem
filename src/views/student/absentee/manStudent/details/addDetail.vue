...
export default {
  ...
  methods: {
    ...
    handlUpdata() {
      ...
      this.detailInfo.xsXjxx.pyccm = this.$route.query.pycc;
      let data = this.detailInfo;
      //0必填和空的校验
      if (!data.xsJbxx.xh) {
        this.$message.error("学号不能为空");
      } else if (!data.xsJbxx.xm) {
        this.$message.error("姓名不能为空");
      } else if (!data.xsJbxx.sfzjh) {
        this.$message.error("身份证件号不能为空");
      } else if (!data.xsJbxx.xbm) {
        this.$message.error("性别不能为空");
      } else if (!data.xsXjxx.bjm) {
        this.$message.error("班级不能为空");
      } else if (!data.xsXjxx.dwh) {
        this.$message.error("培养单位不能为空");
      } else if (!data.xsXjxx.nj) {
        this.$message.error("年级不能为空");
      } else {
        insertRegStuInfo(data)
          .then((res) => {
            this.$message({
              message: res.errmsg,
              type: "success",
