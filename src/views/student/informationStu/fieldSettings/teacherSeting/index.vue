    ...
    <div class="wrap">
      ...
      <div class="detail_right">
        ...
        <div class="headline" id="tag_0">
          ...
          <div class="boxSelect">
            <el-checkbox
              v-model="basicCheckBox.jbxxzd"
              @change="
                (val) => {
                  xsjbxxReader(val, 1, columns.xs_jbxx, 1);
                }
              "
              >全部只读</el-checkbox
            >
            <el-checkbox
              v-model="basicCheckBox.jbxxbt"
              @change="
                (val) => {
                  xsjbxxReader(val, 2, columns.xs_jbxx, 1);
                }
              "
              >全部必填</el-checkbox
            >
          ...
          <el-row :gutter="20">
            ...
            >
              <div class="wrap">
                ...
                <div class="content">
                  <!--- 0 必填 1可写 2可读--->
                  <el-radio v-model="item.approveColumnAuth" label="2"
                    >只读</el-radio
                  >
                  <el-radio v-model="item.approveColumnAuth" label="0"
                    >必填</el-radio
                  >
                  <el-radio v-model="item.approveColumnAuth" label="1"
                    >可写</el-radio
                  >
                </div>
              </div>
        ...
        <div class="headline" id="tag_0">
          ...
          <div class="boxSelect">
            <el-checkbox
              v-model="basicCheckBox.xjxxReader"
              @change="
                (val) => {
                  xsjbxxReader(val, 1, columns.xs_xjxx, 6);
                }
              "
              >全部只读</el-checkbox
            >
            <el-checkbox
              v-model="basicCheckBox.xjxxBt"
              @change="
                (val) => {
                  xsjbxxReader(val, 2, columns.xs_xjxx, 6);
                }
              "
              >全部必填</el-checkbox
            >
        ...
        <div class="headline" id="tag_1">
          ...
          <div class="boxSelect">
            <el-checkbox
              v-model="basicCheckBox.lxfsreader"
              @change="
                (val) => {
                  xsjbxxReader(val, 1, columns.xs_txxx, 2);
                }
              "
              >全部只读</el-checkbox
            >
            <el-checkbox
              v-model="basicCheckBox.lxfsbt"
              @change="
                (val) => {
                  xsjbxxReader(val, 2, columns.xs_txxx, 2);
                }
              "
              >全部必填</el-checkbox
            >
        ...
        <div class="headline" id="tag_2">
          ...
          <div class="boxSelect">
            <el-checkbox
              v-model="basicCheckBox.jtcyxxReader"
              @change="
                (val) => {
                  xsjbxxReader(val, 1, columns.xs_jtcyxx, 3);
                }
              "
              >全部只读</el-checkbox
            >
            <el-checkbox
              v-model="basicCheckBox.jtcyxxBt"
              @change="
                (val) => {
                  xsjbxxReader(val, 2, columns.xs_jtcyxx, 3);
                }
              "
              >全部必填</el-checkbox
            >
        ...
        <div class="information">
          <el-row :gutter="20">
            <el-col
              :span="8"
              class="rowStyle"
              v-for="item in columns.xs_jtcyxx"
              :key="item.id"
            >
              <div class="wrap">
                <div class="title">{{ item.dicCloumnChinese }}</div>
        ...
        <div class="headline" id="tag_3">
          ...
          <div class="boxSelect">
            <el-checkbox
              v-model="basicCheckBox.xxjlReader"
              @change="
                (val) => {
                  xsjbxxReader(val, 1, columns.xs_xxjl, 4);
                }
              "
              >全部只读</el-checkbox
            >
            <el-checkbox
              v-model="basicCheckBox.xxjlBt"
              @change="
                (val) => {
                  xsjbxxReader(val, 2, columns.xs_xxjl, 4);
                }
              "
              >全部必填</el-checkbox
            >
        ...
        <div class="headline" id="tag_4">
          ...
          <div class="boxSelect">
            <el-checkbox
              v-model="basicCheckBox.gzjlReader"
              @change="
                (val) => {
                  xsjbxxReader(val, 1, columns.xs_gzjl, 5);
                }
              "
              >全部只读</el-checkbox
            >
            <el-checkbox
              ...
              @change="
                (val) => {
                  xsjbxxReader(val, 2, columns.xs_gzjl, 5);
                }
              "
              >全部必填</el-checkbox
            >
...
export default {
  ...
  methods: {
    ...
    // 基本信息全部只读
    xsjbxxReader(val, index, data, xm) {
      // val 选中 ，index 0必填 1可写 2可读
    ...
    getUpdateStuColumns() {
      ...
      newData = newData.concat(
        this.columns.xs_gzjl,
        this.columns.xs_jbxx,
        this.columns.xs_xjxx,
        this.columns.xs_jtcyxx,
        this.columns.xs_txxx,
        this.columns.xs_xxjl
      );
      updateStuColumns(JSON.stringify(newData))
        .then((res) => {
          if (res.errcode == "00") {
            this.$message({
              message: res.errmsg,
