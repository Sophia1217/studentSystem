    ...
    <div class="tableWrap mt15">
      <div class="headerTop">
        ...
        <div class="headerRight">
          ...
          <div class="btns borderOrange" @click="handleCommand">
            <i class="icon orangeIcon"></i><span class="title">导出</span>
          </div>
          <div class="btns borderGreen" @click="deleteDialog">
            <i class="icon rollbackIcon"></i
            ><span class="title1">撤销确认</span>
          </div>
        </div>
      </div>
      ...
      <div class="mt15">
        ...
        >
          ...
          <el-table-column prop="dwhmc" label="培养单位" sortable>
          </el-table-column>
      ...
      <el-dialog :title="title2" :visible.sync="showDelete" width="30%">
        <span>确认撤销{{ this.len }}条学生数据？</span>
        <span slot="footer" class="dialog-footer">
          <el-button @click="handleCancelB">取 消</el-button>
          <el-button type="primary" class="confirm" @click="handleDelete()"
            >确 定</el-button
          >
        </span>
      </el-dialog>
...
export default {
  ...
  data() {
    return {
      uploadUrl: process.env.VUE_APP_BASE_API + "/StuStatusChange/importXjyd",
      activeName: "first",
      showExportA: false,
      showDelete: false,
      title: "导出提示",
  ...
  methods: {
    //模板下载
    mbDown() {
      downLoad().then((res) => this.downloadFn(res, "导入模板下载", "xlsx"));
    },
    ///上传
    ...
    handleSearch() {
      ...
      getStuStatusChangePageList(data)
        .then((res) => {
    ...
    async handleCommand() {
      if (this.multipleSelection.length > 0) {
        ...
        this.exportParams = data;
        await getStuStatusChangePageList(data)
          .then((res) => {
    ...
    expTalk() {
      ...
      excelTest(this.exportParams)
        .then((res) => {
          that.downloadFn(res, "学籍异动已确认学生表.xlsx", "xlsx");
        })
        .catch((err) => {});
      this.showExportA = false;
    },
...
.changeList {
  ...
  .tableWrap {
    ...
    .headerTop {
      ...
      //     }
      //     .greenIcon {
      //       background: url("~@/assets/assistantPng/out.png");
      //     }
      //     .importIcon {
      //       background: url("~@/assets/assistantPng/in.png");
      //     }
      //     .confirmIcon {
